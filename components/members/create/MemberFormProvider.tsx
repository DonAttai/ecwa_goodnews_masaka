...
export const MemberFormProvider: React.FC<React.PropsWithChildren<{}>> = ({
  ...
  const next = async () => {
    ...
    const ok = await methods.trigger(keys as any, { shouldFocus: true })
    if (!ok) {
      const errors = Object.keys(methods.formState.errors)
      const currentStepKeys = keys as string[]
      const currentStepHasError = errors.some((field) =>
        currentStepKeys.some(
      ...
      if (firstErrorField) {
        for (let i = 0; i < STEPS.length; i++) {
          if (
            (fieldsForStep[STEPS[i]] as string[]).some(
              (key) =>
                firstErrorField === key || firstErrorField.startsWith(`${key}.`)
            )
          ) {
            setStepIndex(i)
            break
